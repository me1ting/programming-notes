...
让人感兴趣的是，如何使用这个静态类型系统。由于类型在编译时被完全擦除，TS中的类型是JS之上的一层简单的约束。
...
JS存在这些类型，但是无法显式声明类型。注意，这区别于包装类型（或函数）`Number`,`String`,`Boolean`

## 数组

在TS中[数组](https://www.typescriptlang.org/docs/handbook/2/everyday-types.html#arrays)具有显式的类型：`type[]`(如`string[]`)或者泛型的形式`Array<type>`。
...
*TS中频繁使用”匿名“类型，是因为前面我们提到的：类型是约束而不是目的*
...
上面代码中的A,B类型是相同的，A,B只是别名而非定义了其他语言中的真正的类型。
