...
在TS中，**类型是工具而非目的**，这是TypeScript与传统编译型语言如Java,Go等最大的不同，我们还是在写JS，只是加了类型。
...
带类型注释的函数，虽然`参数的类型注释`和`返回值的类型注释`都是可选，但建议都带上。
...
_匿名函数是个特例，通常不需要带上参数类型。_

## 对象类型

与后述的`联合类型`一样，对象类型在形式参数中通常使用`类型字面量`的形式，而非命名的类型。

_TS中”匿名“类型出现的频率较高，是因为前面我们提到的：类型是约束而不是目的_
...
_handbook 使用一篇[长文](https://www.typescriptlang.org/docs/handbook/2/narrowing.html)来讲解**缩小**这一行为的背后逻辑，但比较复杂，除非你确实感到了困惑，不推荐提前去看这部分内容。_
...
复杂使用情况下，类型别名和接口在功能上存在一些区别，但不是本文关注的内容。
...
某些场景，编译器检测无法进行类型转换，但使用者保证可以合法类型转换，需要使用两次`as`：
...
TypeScript此外有独立的[枚举类型](https://www.typescriptlang.org/docs/handbook/enums.html)，这里的“枚举”实现价值在于：更加轻量简洁
...
_undefined在JS也可以看做是一种类型，因为typeof返回值有”undefined“_
...
如果用户能够确定可能为空的类型，其值使用时不为空，那么可以使用后缀`!`运算符让编译器视为非空值（如果错误，会导致运行时异常）。

```typescript
function liveDangerously(x?: number | null) {
  // 编译器不会报错，但运行时如果为null，会抛出异常
