...
在TypeScript中，更多的是在形式参数上使用类型来作为约定，并不强制值也具有明确的类型，而只要其满足形式参数的需求，这是TypeScript与传统编译型语言Java或C#等最大的不同。

# 常见类型
JavaScript中存在类型，但是**不显式使用类型**，而在TypeScript中显式使用类型。

## number,string,boolean
JavaScript存在这些类型，但是无法显式声明类型。

## 数组
在TypeScript中[数组](https://www.typescriptlang.org/docs/handbook/2/everyday-types.html#arrays)具有显式的类型：`type[]`(将type替换为具体的类型，如`string[]`)或者泛型的形式`Array<type>`。
...
```
对于不存在的属性，无法在编译期间进行检查，TypeScript选择交给JavaScript处理：访问不存在的属性会返回`undefined`，可能会导致运行时错误。
...
# 类型断言
[类型断言](https://www.typescriptlang.org/docs/handbook/2/everyday-types.html#type-assertions)是某种意义上的强制类型转换。
...
`常量类型`是TypeScript所**特有**的，类型为常量本身（一般为`string`或`number`），且只有唯一值本身：
...
> TypeScript有独立的[枚举类型](https://www.typescriptlang.org/docs/handbook/enums.html)，这里的“枚举”实现意义在于：更加轻量简洁

## 常量接口
《handbook》的这部分放这里其实不太合理，个人理解这部分包括两个内容：

- 在JavaScript中，`const`用于值为对象类型的变量时，表示变量指向的对象无法改变，而对象的字段是可以改变的，TypeScript沿用该语法规定。
- 变量传递给`常量类型`的形式参数、变量时，需要使用`as const`语法来进行转换。
...
## 非空断言运算符
从示例来看**没啥用处**的运算符：不执行检测，强制认为可能为`null`或`undefined`的变量是非`null`和非`undefined`：
```typescript
...
# 枚举
用到时再[了解](https://www.typescriptlang.org/docs/handbook/enums.html)。
